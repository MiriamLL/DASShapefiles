...
```{r}
German_EEZ<-DASShapefiles::German_EEZ
```
...
```{r}
German_coast<-DASShapefiles::German_coast
```
...
```{r}
EEZ_North<-DASShapefiles::EEZ_North
```
...
```{r}
EEZ_Baltic<-DASShapefiles::EEZ_Baltic
```
...
```{r}
German_natura<-DASShapefiles::German_natura
```
...
```{r}
German_SCA<-DASShapefiles::German_SCA
```
...
```{r}
German_Shipping<-DASShapefiles::German_Shipping
```
...
```{r}
German_OWF<-DASShapefiles::German_OWF
```
...
```{r}
Bath_dataframe<-DASShapefiles::German_bath
```
...
```{r}
DistCoast_dataframe<-DASShapefiles::German_distancecoast
```
...
```{r, eval=TRUE}
Study_areas<-DASShapefiles::Study_areas
```

```{r, eval=TRUE}
library(ggplot2)
DE_land<-st_transform(DASShapefiles::DE_land, 4326)
German_EEZ<-st_transform(DASShapefiles::German_EEZ, 4326)
German_coast<-st_transform(DASShapefiles::German_coast, 4326)
ggplot() +  
...
```{r}
German_ICES<-DASShapefiles::German_ICES
```
...
```{r}
ggplot() +
  geom_sf(data = DASShapefiles::German_EEZ, colour = "red", fill= NA,alpha=0.9, lwd = 0.5)+
  geom_sf(data = DASShapefiles::German_coast, colour = "red", fill= NA,alpha=0.9, lwd = 0.5,linetype="dashed")+
